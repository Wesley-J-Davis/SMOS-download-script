...
# Read from Prep_Config environment settings needed by QFED-2 processing.

( $QFED_BASE = extract_config( "QFED_BASE", $PREP_CONFIG_FILE, "NONE" ) ) ne "NONE"
   or die "(qfed3_driver.pl) ERROR - can not set QFED_BASE configuration value\n";
$ENV{'QFED'} = $QFED_BASE;

( $QFED_PYTHON_PATH = extract_config( "QFED_PYTHON_PATH", $PREP_CONFIG_FILE, "NONE" ) ) ne "NONE"
   or die "(qfed3_driver.pl) WARNING - can not set QFED_PYTHON_PATH configuration value\n";

# Read directory information for input data.

( $QFED_CONFIG = extract_config( "QFED_CONFIG", $PREP_CONFIG_FILE, "NONE" ) ) ne "NONE"
   or print "(qfed3_driver.pl) WARNING - can not set QFED_PYTHON_PATH configuration value\n";

# Get output directories

( $QFED_PERSIST = extract_config( "QFED_PERSIST", $PREP_CONFIG_FILE, "NONE" ) ) ne "NONE"
   or die "(qfed3_driver.pl) ERROR - can not set QFED_PERSIST configuration value\n";

( $QFED_NCKS = extract_config( "QFED_NCKS", $PREP_CONFIG_FILE, "NONE" ) ) ne "NONE"
   or die "(qfed3_driver.pl) ERROR - can not set QFED_NCKS configuration value\n";

( $QFED_OUTPUT_BASE = extract_config( "QFED_OUTPUT_BASE", $PREP_CONFIG_FILE, "NONE" ) ) ne "NONE"
   or die "(qfed3_driver.pl) ERROR - can not set QFED_NCKS configuration value\n";

# Pull NCKS from the modules loaded.
#  $QFED_NCKS="$ENV{'BASEDIR'}/Linux/bin";
...
# If a g5_modules exists in the bin directory, source it.
 ...
 $ENV{'FVROOT'} = ${QFED_BASE};
eval { chdir "${QFED_BASE}/@env" };
print "source g5_modules. ${QFED_BASE}/\@env/g5_modules_perl_wrapper\n";
{
    local @ARGV = ("${QFED_BASE}/\@env");
    do "g5_modules_perl_wrapper";
}
eval { chdir "$QFED_BASE/install/bin"  };
print "module list.\n";
module ("list");
...
$cmd_tmpl = "python qfed_l3a.py --obs vj1 vj2 --config ${QFED_CONFIG} --compress %y4-%m2-%d2";
print "$cmd_tmpl\n";
$cmd = token_resolve("${cmd_tmpl}", $process_date);
print "$cmd\n";

# Remove existing level-3a files, if present.
$file_FRP = token_resolve ("${QFED_OUTPUT_BASE}/FRP/Y%y4/M%m2/",$process_date);
$file_FRP_FCS = token_resolve ("${QFED_OUTPUT_BASE}/FRP_FCS/Y%y4/M%m2/",$process_date);
$file_QFED = token_resolve ("${QFED_OUTPUT_BASE}/QFED/Y%y4/M%m2/",$process_date);

print "/bin/rm ${file_FRP}/*${process_date}*\n ${file_FRP_FCS}/*${process_date}*\n ${file_QFED}/*${process_date}*\n";
system("/bin/rm ${file_FRP}/*${process_date}* ${file_FRP_FCS}/*${process_date}* ${file_QFED}/*${process_date}*");
...
print "QFED_PERSIST = $QFED_PERSIST";
if ($QFED_PERSIST != 0 ) {
    $cmd_tmpl = "python qfed_l3b.py --obs vj1 vj2 --config ${QFED_CONFIG} --ndays ${QFED_PERSIST} --compress %y4-%m2-%d2";
    $cmd = token_resolve("${cmd_tmpl}", $process_date);
}
else {
    $cmd_tmpl = "python qfed_l3b.py --obs vj1 vj2 --config ${QFED_CONFIG} --compress %y4-%m2-%d2";
    $cmd = token_resolve("${cmd_tmpl}", $process_date);
}

print "Run level-3b processing for $process_date with ${QFED_PERSIST}-day persistence.\n";
print "QFED_PERSIST = $QFED_PERSIST";
print "$cmd\n";
$rc=system("$cmd");
